(function ($) {
							...
							if ((state.correct === 5) || (state.correct === 4) || (state.correct === 3)) {
								if (sessionStorage.length >= 6) {
									...
									var $testanother = $("<div>")
										...
										.html('<p><a href="index.html?category=emailsafety">Test your knowledge in another category?</a></p>')
										.appendTo($results_ratio);
								}
							} else {
								$("<button>")
	...
	function tweet(state, opts) {
		...
		var body = (
			"I got " + state.correct +
			" out of " + state.total +
			" on @CDSE's Cybersecurity Trivia Twirl's Email Safety. Play Cybersecurity Trivia Twirl here: " + opts.url
		);
