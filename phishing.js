(function ($) {
	...
	function render(quiz_opts) {
		...
		*/
		...
		$('<h2>')
			.text(quiz_opts.subtitle)
			.attr('class', 'quiz-subtitle')
			.appendTo($title_slide);
		...
		*/
				...
				ans_btn.on('click', function () {
					...
					function next() {
						...
						if (last_question) {
							...
							if ((state.correct === 5) || (state.correct === 4) || (state.correct === 3)) {
								if (sessionStorage.length >= 6) {
									var $congrats = $('<p>')
										.attr('class', 'congrats')
										.html('Congratulations!<br>You have completed the game.<br><a href="support/cookies-clear.html">Play again?</a>')
										.appendTo($results_ratio);
								} else {
									var $testanother = $("<div>")
										.attr('class', 'results-social')
										.html('<p><a href="index.html?category=cyberinsiderthreat">Test your knowledge in another category?</a></p>')
										.appendTo($results_ratio);
								}
							} else {
								$("<button>")
									.attr('class', 'try-button btn')
									.text("Try again?")
									.click(function () {
										state.correct = 0;
										$quiz.carousel(0);
		...
		var $social = $("<div>")
			.attr('class', 'results-social')
			.html('<div id = "social-text">Did you like the game? Share your results with your friends, so they can give it a shot!</div>')
			.appendTo($results_slide);

		var $twitter_link = $('<a target="_blank" aria-label="Twitter Icon">')
			.html('<span class="social social-twitter follow-tw"></span>')
			.appendTo($social);

		var $facebook_link = $('<a target="_blank" aria-label="Facebook Icon">')
	...
	function resultsText(state) {
		...
		switch (true) {
			...
			case (ratio > 0.60):
				...
				break;
			case (ratio > 0.5):
				text = "Well, at least you got about half of them right…";
				break;
	...
	function tweet(state, opts) {
		...
		var body = (
			"I got " + state.correct +
			" out of " + state.total +
			" on @CDSE's Cybersecurity Trivia Twirl's Cyber Insider Threat. Play Cybersecurity Trivia Twirl here: " + opts.url
		);
